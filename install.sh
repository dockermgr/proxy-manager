...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
##@Version           :  202410251519-git
# @@Author           :  Jason Hempstead
# @@Contact          :  jason@casjaysdev.pro
# @@License          :  LICENSE.md
# @@ReadME           :  install.sh --help
# @@Copyright        :  Copyright: (c) 2024 Jason Hempstead, Casjays Developments
# @@Created          :  Friday, Oct 25, 2024 15:19 EDT
# @@File             :  install.sh
# @@Description      :  Container installer script for proxy-manager
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
export APPNAME="proxy-manager"
export VERSION="202410251519-git"
...
__run_pre_install() {
  for service in httpd apache2 nginx caddy traefik; do systemctl disable --now $service >/dev/null 2>&1; done
  return 0
}
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
__show_post_message() {
  ...
  printf_yellow "Nginx Proxy manager is listening on http://$(__public_ip || echo "$HOSTNAME"):10001"
  return 0
}
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set custom script - saves to /config/env/proxy-manager.script.sh
...
if [ -n "$PROXY_MANAGER_USERNAME" ]; then
  CONTAINER_USER_NAME="$PROXY_MANAGER_USERNAME"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
  CONTAINER_USER_NAME="${PROXY_MANAGER_USERNAME:-${CONTAINER_USER_NAME:-$DEFAULT_USERNAME}}"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
...
if [ -n "$PROXY_MANAGER_PASSWORD" ]; then
  CONTAINER_USER_PASS="$PROXY_MANAGER_PASSWORD"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
  ...
  CONTAINER_USER_PASS="${PROXY_MANAGER_PASSWORD:-${CONTAINER_USER_PASS:-$DEFAULT_PASSWORD}}"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
